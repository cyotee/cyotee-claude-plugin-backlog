...
**IMPORTANT:** This command does NOT run automatically. The agent exiting with `<promise>PHASE_DONE</promise>` does NOT trigger this command. The user must explicitly run `/complete` when ready.
...
**Context:** PROMPT.md exists in current directory, NOT running from a worktree. This handles tasks started with `/work`.
...
if [[ "$CURRENT_BRANCH" != "main" && "$CURRENT_BRANCH" != "master" ]]; then
  ...
  if git rebase main; then
    ...
    echo "4. Re-run /complete ${TASK_ID}"
    exit 1
  fi
...
2. Start next task: /work <task-id>
3. Or create worktree: /launch <task-id>
...
For tasks started with `/launch`, use the two-phase worktree workflow:
...
if [[ "$TASK_STATUS" != "Pending Merge" ]]; then
  ...
  echo "Run Phase 1 from the task worktree first: /complete ${TASK_ID}"
  exit 1
fi
...
|------|-------|----------------|
| {PREFIX}-{XXX} | {Title} | /launch {PREFIX}-{XXX} |
| {PREFIX}-{YYY} | {Title} | /launch {PREFIX}-{YYY} |
...
1. Launch next task: /launch <task-id>
