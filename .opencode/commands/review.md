...
**For task definition audits** (checking TASK.md quality), use `/review` instead.
...
1. **If review passed:** `/complete {TASK_ID}` - Mark task complete
...
- **No task ID provided:** "Usage: /review <task-id>"
- **Task not found:** Show available task IDs
- **Task not in progress:** Show current status and suggest appropriate action
- **No worktree exists:** "Task has no worktree. Use /launch first."
- **Already in review:** "Task is already in review mode."

## Related Commands

- `/launch <ID>` - Launch agent for implementation
- `/complete <ID>` - Complete and merge task
- `/from-review <ID>` - Create tasks from review suggestions
