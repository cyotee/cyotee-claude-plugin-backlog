...
if [[ -n "$WORKTREE" ]]; then
  ...
  echo "2. Remove the worktree first with /complete or manually"
  exit 1
fi
```
...
if [[ -f "PROMPT.md" ]]; then
  ...
  if [[ -n "$EXISTING_TASK" && "$EXISTING_TASK" != "$TASK_ID" ]]; then
    ...
    echo ""
    echo "Options:"
    echo "1. Complete the current task first: /complete ${EXISTING_TASK}"
...
- `/complete` - USER-ONLY: marks task complete
- `/review` - USER-ONLY: transitions to review mode
...
**Do NOT invoke /complete or /review yourself.**
...
- **No task ID provided:** "Usage: /work <task-id>"
...
- **No tasks/ directory:** "Run /init first"

## Related Commands

- `/launch <ID>` - Create worktree for isolated work
- `/complete <ID>` - Complete and cleanup in-session task
- `/read <ID>` - Read task details
