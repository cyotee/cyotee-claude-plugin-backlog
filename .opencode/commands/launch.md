...
This registers the launched task in the current session. Note: The agent working in the worktree will be in a separate Claude session and should also create its own Task entry when it starts via `/prompt`.
...
1. **Go to review:** `/review {TASK_ID}` - Transition to code review mode
2. **Complete directly:** `/complete {TASK_ID}` - Skip review and mark complete
...
```bash
$ /launch CRANE-003 --max-iterations 20
